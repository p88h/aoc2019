...
impl FastDeck {
    fn new(size: usize, trace: usize) -> FastDeck {
        FastDeck{ trace: trace % size, size: size }
    }
}
...
impl PolyDeck {
    ...
    fn exp(&self, pow: usize) -> (i128, i128) {
        if pow == 0 { return (1,0) }
        if pow == 1 { return (self.a, self.b) }
        let (a1, b1) = self.exp(pow / 2);
        // a1*(a1*x + b1)+b1
        let mut a2 = (a1 * a1) % self.size as i128;
        let mut b2 = (a1 * b1 + b1) % self.size as i128;
        // a*(a2*x + b2)+b
        if pow % 2 == 1 {
            a2 = (self.a * a2 ) % self.size as i128;
            b2 = (self.a * b2 + self.b ) % self.size as i128;
        }
        return (a2, b2);
    }
    // a*x+b=pos => pos-b=a*x => x=(pos-b)/x (mod size)
    fn compute(&self, pow: usize, pos: usize) -> usize {
        // linear ivnersion...
        let (a, b) = self.exp(pow);
        // println!("({}, {})^{} => ({}, {})", self.a, self.b, pow, a, b);
        let inv = mod_inverse(a, self.size as i128);
        // println!("inv({}) => {}", self.size, inv);
        let mut fv = ((pos as i128 - b) * inv) % self.size as i128;
        if fv < 0 { fv += self.size as i128; }
...
impl Deck for PolyDeck {
    ...
    fn find(&mut self) -> usize {
        let mut fv = ((self.a*self.trace as i128)+self.b)%self.size as i128;
...
fn main() {
    ...
    for i in 0..10 {
        // Apply regular operations every time
        for op in &ops {
